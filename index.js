...
app.use(
    express.static(path.join(__dirname, "includes", "public"), {
        maxAge: "1d",
        etag: true
    })
);
app.use(
    express.static(path.join(__dirname, "includes", "assets"), {
        maxAge: "1d",
        etag: true
    })
);
...
app.get("/", (req, res) => {
    try {
        let html = fs.readFileSync(
            path.join(__dirname, "includes", "public", "portal.html"),
            "utf8"
        );
        html = html.replace(
            "</head>",
            `<script>window.appConfig = ${JSON.stringify(
                global.config,
                null,
                2
            )};</script></head>`
...
app.use((req, res) => {
    try {
        res.status(404).sendFile(
            path.join(__dirname, "includes", "public", "404.html")
        );
    } catch (error) {
